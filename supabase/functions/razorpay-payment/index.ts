...
// Public paths that don't need JWT (webhook is called by Razorpay servers)
const PUBLIC_PATHS = new Set(["webhook"]);

/**
 * Extract and verify user from JWT token
 */
async function getUserFromToken(req: Request): Promise<{ userId: string } | { error: string }> {
  const authHeader = req.headers.get("Authorization");

  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    return { error: "Authorization header missing. Please login first." };
  }

  const token = authHeader.replace("Bearer ", "").trim();

  if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
    return { error: "Server configuration error: Supabase auth not configured" };
  }
  ...
  if (claimsError || !claimsData?.claims?.sub) {
    console.error("[Razorpay] JWT verification failed:", claimsError);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (path === "confirm-order") {
      return handleConfirmOrder(body, userId);
    }
...
async function handleConfirmOrder(body: any, userId: string) {
  ...
  // Step 2: Check payment status with Razorpay API
